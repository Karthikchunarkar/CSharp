namespace lists ;
 using CSharp.util; using classes; using d3e.core; using graphql.language; using io.reactivex.rxjava3.core; using io.reactivex.rxjava3.disposables; using java.lang; using lists; using models; using store;  public class ReviewEventListSubscriptionHelper :  FlowableOnSubscribe < DataQueryDataChange > {  private class Output { public Dictionary < long,Row > rows { get; private set; } = MapExt.Dictionary() ;
 ...
 private ReviewEventListImpl reviewEventListImpl ;
 ...
 private ReviewEventListRequest inputs ;
 
 public ReviewEventListSubscriptionHelper (  TransactionWrapper transactional, ReviewEventListImpl reviewEventListImpl, D3ESubscription subscription ) {
 this.transactional = transactional ;
 this.reviewEventListImpl = reviewEventListImpl ;
 ...
 try {
 dataEvent.Data = reviewEventListImpl.getAsJson(field,dataEvent.NativeData,dataEvent.Count) ;
 ...
 change.nativeData = reviewEventListImpl.GetNativeResult(this.inputs) ;
 ...
 public Flowable < DataQueryDataChange > subscribe (  Field field, ReviewEventListRequest inputs ) {
 {
 ...
 row.set(0,event.getReferenceID()) ;
