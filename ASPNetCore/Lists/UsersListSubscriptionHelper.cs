namespace lists ;
 using CSharp.util; using classes; using d3e.core; using graphql.language; using io.reactivex.rxjava3.core; using io.reactivex.rxjava3.disposables; using java.lang; using java.util; using lists; using models; using store;  public class UsersListSubscriptionHelper :  FlowableOnSubscribe < DataQueryDataChange > {  private class Output { public Dictionary < long,Row > rows { get; private set; } = MapExt.Dictionary() ;
 ...
 private UsersListImpl usersListImpl ;
 ...
 private UsersListRequest inputs ;
 
 public UsersListSubscriptionHelper (  TransactionWrapper transactional, UsersListImpl usersListImpl, D3ESubscription subscription ) {
 this.transactional = transactional ;
 this.usersListImpl = usersListImpl ;
 ...
 try {
 dataEvent.Data = usersListImpl.getAsJson(field,dataEvent.NativeData,dataEvent.Count) ;
 ...
 change.nativeData = usersListImpl.GetNativeResult(this.inputs) ;
 ...
 public Flowable < DataQueryDataChange > subscribe (  Field field, UsersListRequest inputs ) {
 {
 ...
 // A listener is added by default on the Table from which we pull the data, since any change in that must trigger a subscription change.
  Disposable baseSubscribe=((Flowable < D3ESubscriptionEvent<Admin> >)subscription.onAdminChangeEvent()).Subscribe((  e ) => ApplyAdmin(e));
 disposables.Add(baseSubscribe) ;
 }
 public void ApplyAdmin (  D3ESubscriptionEvent < Admin > e ) {
 ...
 private void CreateInsertChange (  java.util.List < DataQueryDataChange > changes, Admin model ) {
  DataQueryDataChange change=new DataQueryDataChange();
 change.nativeData = createAdminData(model) ;
 ...
 private void CreateUpdateChange (  java.util.List < DataQueryDataChange > changes, Admin model ) {
 ...
 private void CreateDeleteChange (  Csharp.util.List < DataQueryDataChange > changes, Admin model ) {
  Row row=output.get(model.getId());
 if ( row == null ) {
 return ;
 }
 ...
 private bool createPathChangeChange (  List < DataQueryDataChange > changes, Admin model, Admin old ) {
  string oldValue0;
  string modelValue0;
 ...
 private java.util.List < NativeObj > createAdminData (  Admin admin ) {
  java.util.List < NativeObj > data=ListExt.List();
  NativeObj row=new NativeObj(2);
 row.set(1,admin.getId()) ;
 row.setId(1) ;
 data.Add(row) ;
 return data ;
 }
