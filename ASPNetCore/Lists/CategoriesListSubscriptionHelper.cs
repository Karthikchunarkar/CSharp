namespace lists ;
 ...
 rows[Id().wrappedBase] null row ;
  NativeObj wrappedBase=wrappedBase.getRef(1);
  string OrderByValue0=wrappedBase.getString(0);
 ...
 private CategoriesListImpl categoriesListImpl ;
 ...
 private CategoriesListRequest inputs ;
 
 public CategoriesListSubscriptionHelper (  TransactionWrapper transactional, CategoriesListImpl categoriesListImpl, D3ESubscription subscription ) {
 this.transactional = transactional ;
 this.categoriesListImpl = categoriesListImpl ;
 ...
 try {
 dataEvent.Data = categoriesListImpl.getAsJson(field,dataEvent.NativeData,dataEvent.Count) ;
 ...
 change.nativeData = categoriesListImpl.GetNativeResult(this.inputs) ;
 ...
 public Flowable < DataQueryDataChange > subscribe (  Field field, CategoriesListRequest inputs ) {
 {
 ...
 // A listener is added by default on the Table from which we pull the data, since any change in that must trigger a subscription change.
  Disposable baseSubscribe=((Flowable < D3ESubscriptionEvent<Category> >)subscription.onCategoryChangeEvent()).Subscribe((  e ) => ApplyCategory(e));
 disposables.Add(baseSubscribe) ;
 }
 public void ApplyCategory (  D3ESubscriptionEvent < Category > e ) {
  System.Collections.Generic.List < DataQueryDataChange > changes=ListExt.List();
  Category model=e.model;
 ...
 private void CreateInsertChange (  java.util.List < DataQueryDataChange > changes, Category model ) {
  DataQueryDataChange change=new DataQueryDataChange();
 change.nativeData = createCategoryData(model) ;
 ...
 private void CreateUpdateChange (  java.util.List < DataQueryDataChange > changes, Category model ) {
 ...
 private void CreateDeleteChange (  Csharp.util.List < DataQueryDataChange > changes, Category model ) {
  Row row=output.get(model.getId());
 if ( row == null ) {
 return ;
 }
 ...
 private bool createPathChangeChange (  List < DataQueryDataChange > changes, Category model, Category old ) {
  string oldValue0;
  string modelValue0;
 ...
 private java.util.List < NativeObj > createCategoryData (  Category category ) {
  java.util.List < NativeObj > data=ListExt.List();
  NativeObj row=new NativeObj(2);
 row.set(1,category.getId()) ;
 row.setId(1) ;
 data.Add(row) ;
 return data ;
 }
