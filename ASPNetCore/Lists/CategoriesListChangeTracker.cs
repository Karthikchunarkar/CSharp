namespace lists ;
 using classes; using d3e.core; using gqltosql2; using io.reactivex.rxjava3.disposables; using io.reactivex.rxjava3.functions; using java.util.stream; using lists; using models; using rest.ws; using store;  public class CategoriesListChangeTracker :  Cancellable {  private class OrderBy { public string OrderByValue0 
 ...
 }  private class CategoriesListInput { private int pageSize 
 private int offset 
 private string orderBy 
 private bool ascending 
 public CategoriesListInput (  CategoriesListRequest _req ) {
 ...
 private CategoriesListInput inputs ;
 
 private Field field ;
 
 public CategoriesListChangeTracker (  DataChangeTracker tracker, ClientSession clientSession, Field field ) {
 this.tracker = tracker ;
 this.clientSession = clientSession ;
 this.field = field ;
 }
 public void Init (  OutObject out, CategoriesList initialData, CategoriesListRequest inputs ) {
 {
 ...
 this.inputs = new CategoriesListInput(inputs) ;
 ...
 private void StoreInitialData (  CategoriesList initialData ) {
 root = initialData ;
 ...
 // A listener is added by default on the Table from which we pull the data, since any change in that must trigger a subscription change.
  Disposable baseSubscribe=tracker.listen(SchemaConstants.Category,null,(  obj, old, type ) => applyCategory(((Category)obj),((Category)obj),type));
 ...
 public void applyCategory (  Category model, Category old, StoreEventType type ) {
 ...
 private void CreateInsertChange (  Category model ) {
  long id=model.Id;
 ...
 private void CreateDeleteChange (  Category model ) {
  long id=model.getId();
 ...
 private bool CreatePathChangeChange (  Category model, Category old ) {
  string oldValue0;
  string modelValue0;
 ...
 private void moveItems (  Category model, TypeAndId data, string OrderByValue0 ) {
 // Find old index
