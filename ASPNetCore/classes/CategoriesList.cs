namespace classes ;
 using classes; using d3e.core; using java.util; using lists; using models; using store;  public class CategoriesList :  DBObject { public static int _STATUS = 0 ;
 ...
 private int count 
 private List < Category > items = new D3EPersistanceList<>(this,_ITEMS) ;
 
 private List < TypeAndId > itemsRef 
 public CategoriesList (  ) {
 }
 public CategoriesList (  int count, List < string > errors, List < Category > items, ResultStatus status ) {
 this.Count = count ;
 ...
 ((D3EPersistanceList < Category >)this.items).setAll(items) ;
 }
 ...
 public void addToItems (  Category val, long index ) {
 if ( index == -1 ) {
 this.items.add(val) ;
 }
 else {
 ...
 public void removeFromItems (  Category val ) {
 ...
 public int _typeIdx (  ) {
 return SchemaConstants.CategoriesList ;
 }
 public String _type (  ) {
