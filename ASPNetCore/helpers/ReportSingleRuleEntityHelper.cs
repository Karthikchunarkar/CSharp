namespace helpers ;
 using d3e.core; using helpers; using models; using store;  public class ReportSingleRuleEntityHelper < T > :  ReportRuleEntityHelper<T> where T : ReportSingleRule { public ReportSingleRuleEntityHelper (  IEntityMutator mutator ) {
  mutator=mutator;
 }
 ...
 return ((T)provider.get().find(SchemaConstants.ReportSingleRule,id)) ;
 }
