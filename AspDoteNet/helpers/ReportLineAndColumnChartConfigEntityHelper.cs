namespace helpers ;
 using EntityValidationContext = store.EntityValidationContext; using IEntityMutator = store.IEntityMutator; using ReportBaseConfigEntityHelper = helpers.ReportBaseConfigEntityHelper; using ReportField = models.ReportField; using ReportFieldEntityHelper = helpers.ReportFieldEntityHelper; using ReportLineAndColumnChartConfig = models.ReportLineAndColumnChartConfig; using SchemaConstants = d3e.core.SchemaConstants;  public class ReportLineAndColumnChartConfigEntityHelper < T > :  ReportBaseConfigEntityHelper<T> where T : ReportLineAndColumnChartConfig { public ReportLineAndColumnChartConfigEntityHelper (  IEntityMutator mutator ) {
  mutator=mutator;
 }
