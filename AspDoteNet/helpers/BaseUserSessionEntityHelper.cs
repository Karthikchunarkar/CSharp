namespace helpers ;
 using BaseUserSession = models.BaseUserSession; using BaseUserSessionRepository = repository.jpa.BaseUserSessionRepository; using EntityHelper = store.EntityHelper; using EntityValidationContext = store.EntityValidationContext; using IDFileRepository = store.IDFileRepository; using IEntityMutator = store.IEntityMutator; using string = System.string;  public class BaseUserSessionEntityHelper < T > :  EntityHelper<T> where T : BaseUserSession { private readonly IEntityMutator mutator ;
