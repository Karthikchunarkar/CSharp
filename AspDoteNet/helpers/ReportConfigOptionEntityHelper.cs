namespace helpers ;
 using EntityHelper = store.EntityHelper; using EntityValidationContext = store.EntityValidationContext; using IDFileRepository = store.IDFileRepository; using IEntityMutator = store.IEntityMutator; using ReportConfigOption = models.ReportConfigOption; using ReportConfigOptionRepository = repository.jpa.ReportConfigOptionRepository; using ReportConfigRepository = repository.jpa.ReportConfigRepository; using string = System.string;  public class ReportConfigOptionEntityHelper < T > :  EntityHelper<T> where T : ReportConfigOption { private readonly IEntityMutator mutator ;
