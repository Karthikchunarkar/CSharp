namespace helpers ;
 using AnonymousUser = models.AnonymousUser; using AnonymousUserRepository = repository.jpa.AnonymousUserRepository; using BaseUserEntityHelper = helpers.BaseUserEntityHelper; using EntityValidationContext = store.EntityValidationContext; using IDFileRepository = store.IDFileRepository; using IEntityMutator = store.IEntityMutator;  public class AnonymousUserEntityHelper < T > :  BaseUserEntityHelper<T> where T : AnonymousUser { private readonly AnonymousUserRepository anonymousUserRepository ;
