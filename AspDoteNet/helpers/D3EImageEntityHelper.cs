namespace helpers ;
 using AvatarRepository = repository.jpa.AvatarRepository; using D3EImage = models.D3EImage; using EntityHelper = store.EntityHelper; using EntityValidationContext = store.EntityValidationContext; using IDFileRepository = store.IDFileRepository; using IEntityMutator = store.IEntityMutator; using int = System.int;  public class D3EImageEntityHelper < T > :  EntityHelper<T> where T : D3EImage { private readonly IEntityMutator mutator ;
