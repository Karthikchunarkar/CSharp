namespace helpers ;
 using EntityHelper = store.EntityHelper; using EntityValidationContext = store.EntityValidationContext; using ID3EEntityManagerProvider = store.ID3EEntityManagerProvider; using IEntityMutator = store.IEntityMutator; using ReportRule = models.ReportRule; using SchemaConstants = d3e.core.SchemaConstants;  public class ReportRuleEntityHelper < T > :  EntityHelper<T> where T : ReportRule { private readonly IEntityMutator mutator ;
