namespace helpers ;
 using EntityValidationContext = store.EntityValidationContext; using IEntityMutator = store.IEntityMutator; using ReportBaseConfigEntityHelper = helpers.ReportBaseConfigEntityHelper; using ReportCardConfig = models.ReportCardConfig; using ReportFieldEntityHelper = helpers.ReportFieldEntityHelper; using SchemaConstants = d3e.core.SchemaConstants;  public class ReportCardConfigEntityHelper < T > :  ReportBaseConfigEntityHelper<T> where T : ReportCardConfig { public ReportCardConfigEntityHelper (  IEntityMutator mutator ) {
  mutator=mutator;
 }
