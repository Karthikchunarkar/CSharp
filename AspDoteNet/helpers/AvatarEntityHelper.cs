namespace helpers ;
 using Avatar = models.Avatar; using AvatarRepository = repository.jpa.AvatarRepository; using D3EImageEntityHelper = helpers.D3EImageEntityHelper; using EntityHelper = store.EntityHelper; using EntityValidationContext = store.EntityValidationContext; using IDFileRepository = store.IDFileRepository; using IEntityMutator = store.IEntityMutator;  public class AvatarEntityHelper < T > :  EntityHelper<T> where T : Avatar { private readonly IEntityMutator mutator ;
