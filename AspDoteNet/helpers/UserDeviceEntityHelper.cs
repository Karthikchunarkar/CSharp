namespace helpers ;
 using BaseUserRepository = repository.jpa.BaseUserRepository; using EntityHelper = store.EntityHelper; using EntityValidationContext = store.EntityValidationContext; using IDFileRepository = store.IDFileRepository; using IEntityMutator = store.IEntityMutator; using PushNotification = models.PushNotification; using PushNotificationRepository = repository.jpa.PushNotificationRepository; using UserDevice = models.UserDevice; using UserDeviceRepository = repository.jpa.UserDeviceRepository;  public class UserDeviceEntityHelper < T > :  EntityHelper<T> where T : UserDevice { private readonly IEntityMutator mutator ;
