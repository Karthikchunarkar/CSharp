namespace helpers ;
 using AllDevicesRequest = models.AllDevicesRequest; using EntityHelper = store.EntityHelper; using EntityValidationContext = store.EntityValidationContext; using IEntityMutator = store.IEntityMutator;  public class AllDevicesRequestEntityHelper < T > :  EntityHelper<T> where T : AllDevicesRequest { private readonly IEntityMutator mutator ;
 
 public AllDevicesRequestEntityHelper (  IEntityMutator mutator ) {
  mutator=mutator;
 }
