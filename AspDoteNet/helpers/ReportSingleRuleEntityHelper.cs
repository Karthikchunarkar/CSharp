namespace helpers ;
 using EntityValidationContext = store.EntityValidationContext; using IEntityMutator = store.IEntityMutator; using ReportRuleEntityHelper = helpers.ReportRuleEntityHelper; using ReportSingleRule = models.ReportSingleRule; using SchemaConstants = d3e.core.SchemaConstants;  public class ReportSingleRuleEntityHelper < T > :  ReportRuleEntityHelper<T> where T : ReportSingleRule { public ReportSingleRuleEntityHelper (  IEntityMutator mutator ) {
  mutator=mutator;
 }
