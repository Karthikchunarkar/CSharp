namespace helpers ;
 using EntityValidationContext = store.EntityValidationContext; using IEntityMutator = store.IEntityMutator; using ReportFilterEntityHelper = helpers.ReportFilterEntityHelper; using ReportPropertyFilter = models.ReportPropertyFilter; using SchemaConstants = d3e.core.SchemaConstants;  public class ReportPropertyFilterEntityHelper < T > :  ReportFilterEntityHelper<T> where T : ReportPropertyFilter { public ReportPropertyFilterEntityHelper (  IEntityMutator mutator ) {
  mutator=mutator;
 }
