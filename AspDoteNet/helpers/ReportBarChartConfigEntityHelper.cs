namespace helpers ;
 using EntityValidationContext = store.EntityValidationContext; using IEntityMutator = store.IEntityMutator; using ReportBarChartConfig = models.ReportBarChartConfig; using ReportBarChartType = classes.ReportBarChartType; using ReportBaseConfigEntityHelper = helpers.ReportBaseConfigEntityHelper; using ReportField = models.ReportField; using ReportFieldEntityHelper = helpers.ReportFieldEntityHelper; using SchemaConstants = d3e.core.SchemaConstants;  public class ReportBarChartConfigEntityHelper < T > :  ReportBaseConfigEntityHelper<T> where T : ReportBarChartConfig { public ReportBarChartConfigEntityHelper (  IEntityMutator mutator ) {
  mutator=mutator;
 }
