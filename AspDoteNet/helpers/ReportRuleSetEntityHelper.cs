namespace helpers ;
 using EntityValidationContext = store.EntityValidationContext; using IEntityMutator = store.IEntityMutator; using ReportRepository = repository.jpa.ReportRepository; using ReportRule = models.ReportRule; using ReportRuleEntityHelper = helpers.ReportRuleEntityHelper; using ReportRuleSet = models.ReportRuleSet; using SchemaConstants = d3e.core.SchemaConstants;  public class ReportRuleSetEntityHelper < T > :  ReportRuleEntityHelper<T> where T : ReportRuleSet { private readonly ReportRepository reportRepository ;
