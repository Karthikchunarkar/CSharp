namespace helpers ;
 using EntityHelper = store.EntityHelper; using EntityValidationContext = store.EntityValidationContext; using ID3EEntityManagerProvider = store.ID3EEntityManagerProvider; using IEntityMutator = store.IEntityMutator; using ReportNamedCondition = models.ReportNamedCondition; using ReportRuleEntityHelper = helpers.ReportRuleEntityHelper; using SchemaConstants = d3e.core.SchemaConstants;  public class ReportNamedConditionEntityHelper < T > :  EntityHelper<T> where T : ReportNamedCondition { private readonly IEntityMutator mutator ;
