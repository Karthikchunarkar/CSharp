namespace helpers ;
 using EntityHelper = store.EntityHelper; using EntityValidationContext = store.EntityValidationContext; using IEntityMutator = store.IEntityMutator; using VerificationDataByTokenRequest = models.VerificationDataByTokenRequest;  public class VerificationDataByTokenRequestEntityHelper < T > :  EntityHelper<T> where T : VerificationDataByTokenRequest { private readonly IEntityMutator mutator ;
 
 public VerificationDataByTokenRequestEntityHelper (  IEntityMutator mutator ) {
  mutator=mutator;
 }
