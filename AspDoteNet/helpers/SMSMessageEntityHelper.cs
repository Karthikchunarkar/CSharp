namespace helpers ;
 using EntityHelper = store.EntityHelper; using EntityValidationContext = store.EntityValidationContext; using IEntityMutator = store.IEntityMutator; using SMSMessage = models.SMSMessage;  public class SMSMessageEntityHelper < T > :  EntityHelper<T> where T : SMSMessage { private readonly IEntityMutator mutator ;
 
 public SMSMessageEntityHelper (  IEntityMutator mutator ) {
  mutator=mutator;
 }
