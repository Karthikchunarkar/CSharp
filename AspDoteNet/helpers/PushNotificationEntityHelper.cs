namespace helpers ;
 using EntityHelper = store.EntityHelper; using EntityValidationContext = store.EntityValidationContext; using IDFileRepository = store.IDFileRepository; using IEntityMutator = store.IEntityMutator; using PushNotification = models.PushNotification; using PushNotificationRepository = repository.jpa.PushNotificationRepository;  public class PushNotificationEntityHelper < T > :  EntityHelper<T> where T : PushNotification { private readonly IEntityMutator mutator ;
