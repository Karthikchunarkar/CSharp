namespace helpers ;
 using EntityValidationContext = store.EntityValidationContext; using IEntityMutator = store.IEntityMutator; using ReportBaseConfigEntityHelper = helpers.ReportBaseConfigEntityHelper; using ReportField = models.ReportField; using ReportFieldEntityHelper = helpers.ReportFieldEntityHelper; using ReportScatterChartConfig = models.ReportScatterChartConfig; using SchemaConstants = d3e.core.SchemaConstants;  public class ReportScatterChartConfigEntityHelper < T > :  ReportBaseConfigEntityHelper<T> where T : ReportScatterChartConfig { public ReportScatterChartConfigEntityHelper (  IEntityMutator mutator ) {
  mutator=mutator;
 }
