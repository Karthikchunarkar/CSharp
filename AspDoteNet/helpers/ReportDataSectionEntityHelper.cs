namespace helpers ;
 using EntityHelper = store.EntityHelper; using EntityValidationContext = store.EntityValidationContext; using IEntityMutator = store.IEntityMutator; using ReportDataSection = models.ReportDataSection;  public class ReportDataSectionEntityHelper < T > :  EntityHelper<T> where T : ReportDataSection { private readonly IEntityMutator mutator ;
